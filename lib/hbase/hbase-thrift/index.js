...
*/
console.log("host: options.host :: " + options.host + ":" + options.port + " :: set in lib/hbase/hbase-thrift/index.js ");
...
if(self.pool.length < 1 && attempts == undefined){
console.log("0 - self.pool.length: " + i);
//i = 1;

}
    if (!attempts) {
      attempts = 0;
console.log("1 - self.pool.length: " + i);
//i = 1;
    } else if (attempts > 100) {
      cb('unable to get open connection');
      return;
    }
console.log("2 - self.pool.length: " + i);
//i = 1;
    ...
    while (i--) {
      if (self.pool[i].client &&
        ...
        cb(null, self.pool[i]);
        self.log.info("# connections:", self.pool.length, ' - current:', i);
        return;
      }
    }
...
console.log("openNewSocket(" + i + "): " + server.host + " " + server.port);
...
console.log("thrift connect error");
